...
The semantic vocabularies remain exactly80/110/110 strings. **Background handling is unresolved, not fixed to zero.** The pinned text demos append exactly one trailing U+0020 space; the selected config instead evaluates LVIS through LoadText and a1203-entry text file without blanks. The available80-entry COCO text file also has no blank. The V2.1 discussion does not identify which blank convention produced the selected checkpoint's COCO baseline. P1 therefore supersedes P0's provisional no-extra-blank wording with an explicit BLOCKED state: do not choose zero/one/another count and do not resolve the choice by running models. If Lead later resolves a fixed background input, use its same count/placement across all three vocabularies and exclude it from canonical/distractor semantic metrics. Hardness remains the existing frozen BERT-ranked intervention; do not rerank candidates with CLIP to favor YOLO.
...
| Metrics |Same pycocotools2.0.8 AP50,mAP50:95,AR,AR50 atmaxDet100, areaall; filter distractors only after native prediction selection for canonical AP, without rerunning selection on canonical classes alone. Diagnostics atscore>=.25/IoU>=.5: canonicalFP/image via officialmatching, off-canonical predictions/image, microclasscorrect/localizationrecall.|
...
P1 closes native postprocessing by the explicit pre-outcome Lead decision and preserves background ambiguity as a blocker. A future authorized phase must first resolve that ambiguity, reproduce the official zero-shot baseline and establish an approved YOLO audit wrapper before any interaction evaluation. No Grounding-DINO code, environment, cached predictions or frozen plan changes are allowed for this preparation.
