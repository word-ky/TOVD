...
## 2026-09-13 — T013-OPS5 review / T013-OPS6 assignment

**Decision:** OPS5 ACCEPTED; CURRENT MARGIN EROSION IS NOT ATTRIBUTABLE TO OTHER TOVD PROJECT CONTENT IN THE MEASURED INTERVAL; STOP BOUNDARY-CHASING AND USE A LOW-I/O FIXED-GATE SURVIVAL WATCH.

Reviewed evidence:
- task-start `b3b27c6258bda129968cac10e74a8080fed392a9`;
- `86f4d61e139488fd1d5870bbab45c1f9eecc6b36` — finalized OPS5 implementation/evidence;
- `8099c9ae831a4fe4a3ad76c4a34eecb35adbab32` — Codex handoff;
- `research_log/t013/PRIMARY_PROJECT_STORAGE_ATTRIBUTION.md`, helper/tests, raw A/B transcripts and machine receipt;
- current `AGENTS.md`, `coordination/PROTOCOL.md`, Codex mailbox and authoritative OPS5 package.

OPS5 is accepted as descriptive accounting only. The helper is narrowly scoped, imports the accepted OPS2 guard and fixed binding, and passes 5 tests / 32 deterministic fixtures including zero/positive/negative residuals, binding/time/progress failures, malformed/missing evidence and unhealthy states. Exactly two live snapshots were collected; no cache `du`, scientific/prediction access, active-file hashing/mutation, FIN1/replay/scientific analysis, cleanup/restart/resume, YOLO runtime or T014 occurred.

At A (`567/1000`) free was `18,642,763,776`, project `du` `16,055,738,368`, run `du` `9,255,936,000`, required `17,088,163,021`, margin `1,554,600,755`. At B (`578/1000`) free was `18,248,122,368`, project `du` `16,233,095,168`, run `du` `9,433,272,320`, required `16,872,272,692`, margin `1,375,849,676`; both were SAFE / PRIMARY_RUNNING with the same writer/tmux and no wrapper/result by metadata-only checks.

Over 18m22s, free space fell `394,641,408` bytes. Project growth was `177,356,800`, almost entirely active-run growth `177,336,320`; other TOVD project growth was only `20,480`. The accounting residual outside the TOVD project was `217,284,608`. Required free fell `215,890,329` bytes as 11 images completed, so without outside-project pressure the fixed-rule margin would have improved by ~`38.6 MB`; actual margin instead fell `178,751,079` bytes. This is sequential non-atomic accounting, not writer identification, and does not justify a new safety rule or cleanup action.

**Research implication:** enough attribution has now been done. Repeatedly widening `du` scope would be operationally invasive and scientifically irrelevant while the unique preregistered primary is still incomplete. The accepted OPS2 inequality remains the only storage gate. With only ~1.28 GiB margin at the latest endpoint, the safest high-value next hour is low-I/O observation of that fixed gate, not more engineering or YOLO preparation.

**Next action:** `T013-OPS6` is the single active 45–60 minute package in `coordination/CHATGPT_TO_CODEX.md` (instruction commit `c82850b1fa7261dfe2896cfe333af85e0df5b1dc`). Reuse accepted OPS2/OPS3 unchanged and collect at most four ordinary ~15-minute scalar health points across one hour: progress, writer/tmux/wrapper state, `df` free bytes, analysis-result existence only, and exact OPS2 required/margin/status. No `du`, new helper, new threshold, forecast, cleanup, FIN1/replay, scientific access, YOLO or T014. Any fixed-rule/process/completion return-to-Lead state stops immediately after preserving an OPS3 incident snapshot; otherwise stop after the one-hour watch and report.