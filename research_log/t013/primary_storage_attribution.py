"""OPS4: two-snapshot allocated-byte accounting; no scientific I/O."""
import json
import re
...
def snapshot(raw):
    try:
        ...
        totals = {}
        for key, path in (("run_du", RUN), ("cache_du", CACHE)):
        ...
        if totals["cache_du"] > totals["run_du"]:
            raise ValueError("cache allocated size exceeds run")
...
def attribute(raw_a, raw_b):
    ...
    cache = b["cache_du"] - a["cache_du"]
    return {"delta_images": images, "free_consumed": free, "active_run_growth": run,
            "cache_growth": cache, "noncache_run_growth": run - cache,
            "outside_run_pressure": free - run,
            "cache_growth_per_new_image": cache / images if images > 0 else None}
